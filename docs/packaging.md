...
安装阶段先查询 `ddhid63340` 服务：已运行则直接复用；已注册但未运行则跳过自动重装并提示重启检查。只有未注册时才在正确目录调用厂商 `ddc.exe`，收集输出，检查返回码与服务注册，避免重复卸载正在使用的设备。3010 或超时会提示需要重启；失败不假称驱动已可用。应用卸载不会移除共享驱动，不关闭内存完整性、安全启动、签名检查。
